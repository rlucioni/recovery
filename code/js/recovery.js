...
var activeCounty, activeDimension, allCountyData, arc, axis, bb, blockContextMenu, brush, canvasHeight, canvasWidth, color, colorDomains, compressData, compressedData, constant, containsAll, counties, countyAdded, dimension, dimensions, drawPC, drawVisualization, firstTime, formatPercent, graphContainer, graphFrame, graphLine, graphMask, graphXAxis, graphXScale, graphYAxis, graphYScale, graphedCountyId, labels, line, loadingContainer, mapContainer, mapFrame, mapMask, mapX, mapY, meter, modifyGraph, nationalData, parseDate, path, pcFrame, pcPath, pcScales, pcbackground, pcforeground, pcnational, pcx, pcy, progress, projection, resetChoropleth, scaleY, standardMargin, svg, text, timeSlice, total, twoPi, usGeo, windowHeight, windowWidth, zeroes, zoomChoropleth, _i, _len, _ref, _ref1, _ref2;
...
drawPC = function() {
  var add, addedData, allCountyValues, countyData, dimensionExtent, _j, _k, _l, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o;
  allCountyValues = {};
  for (_j = 0, _len1 = dimensions.length; _j < _len1; _j++) {
    dimension = dimensions[_j];
    allCountyValues[dimension] = [];
  }
  for (_k = 0, _len2 = compressedData.length; _k < _len2; _k++) {
    countyData = compressedData[_k];
    addedData = {};
    add = true;
  ...
  d3.selectAll(".pcaxis").each(function(d) {
...
drawVisualization = function(firstTime) {
  var backgroundCounties, g, nationalPoints, nationalValues;
  ...
  if (firstTime) {
    ...
    g.append("g").attr("class", "pcaxis").append("text").attr("text-anchor", "end").attr("x", bb.pc.width).attr("y", -9).text(function(d) {
      return labels[d];
    });
    g.append("g").attr("class", "pcbrush").each(function(d) {
      return d3.select(this).call(pcx[d].brush = d3.svg.brush().x(pcx[d]).on("brush", brush));
