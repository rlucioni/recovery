...
var activeCounty, activeDimension, allCountyData, bb, blockContextMenu, canvasHeight, canvasWidth, color, colorDomains, constant, counties, countyAdded, dates, dimensions, drawVisualization, firstTime, graphFrame, graphLine, graphXAxis, graphXScale, graphYAxis, graphYScale, graphedCountyId, labels, mapContainer, mapFrame, mapMask, mapX, mapY, margin, modifyGraph, nationalData, parseDate, path, pcFrame, projection, resetChoropleth, scaleY, svg, usGeo, zeroes, zoomChoropleth, _ref, _ref1;
...
drawVisualization = function(firstTime) {
  ...
  if (firstTime) {
    ...
    color.domain(colorDomains[activeDimension]);
    timeSlice = allCountyData[0].properties[activeDimension].length - 1;
    counties.transition().duration(1000).style("fill", function(d) {
      var countyData;
      countyData = d.properties[activeDimension];
      if (countyData.length === 0) {
        return "#d9d9d9";
