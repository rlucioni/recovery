...
drawVisualization = function(firstTime) {
  var allCountyValues, allValues, backgroundCounties, brush, brushed, count, countyData, dimensionExtent, g, handle, keyBoxPadding, keyBoxRatio, keyBoxSize, moveBrush, nationalValues, point, roundedPosition, slider, sliderScale, swatch, timeslice, update, _j, _k, _l, _len1, _len2, _len3, _len4, _len5, _len6, _len7, _len8, _m, _n, _o, _p, _q, _ref3, _ref4, _ref5;
  ...
  if (firstTime) {
    ...
    if (activeData !== null) {
      countyLine.datum(activeData.properties[activeDimension]).transition().duration(constant.graphDurationDimSwitch).attr("d", graphLine);
      return countyPoints.data(activeData.properties[activeDimension]).transition().duration(constant.graphDurationDimSwitch).attr("transform", function(d, i) {
        return "translate(" + (graphXScale(nationalData.dates[i])) + ", " + (graphYScale(d)) + ")";
      });
    }
  }
