...
var activeCounty, activeDimension, allCountyData, bb, blockContextMenu, canvasHeight, canvasWidth, color, colorDomains, constant, counties, countyAdded, dimensions, drawPC, drawVisualization, firstTime, graphContainer, graphFrame, graphLine, graphMask, graphXAxis, graphXScale, graphYAxis, graphYScale, graphedCountyId, labels, loadingContainer, mapContainer, mapFrame, mapMask, mapX, mapY, meter, modifyGraph, nationalData, parseDate, path, pcFrame, pcScales, projection, resetChoropleth, scaleY, standardMargin, svg, text, timeSlice, usGeo, windowHeight, windowWidth, zeroes, zoomChoropleth, _ref, _ref1;
...
_ref1 = [null, null, null], allCountyData = _ref1[0], counties = _ref1[1], timeSlice = _ref1[2];

drawVisualization = function(firstTime) {
  var backgroundCounties, brush, brushed, handle, nationalValues, slider, sliderScale;
  ...
  if (firstTime) {
    ...
    handle = slider.append("circle").attr("class", "handle").attr("r", 7);
    ...
    graphFrame.selectAll(".point.national").data(nationalData[activeDimension]).transition().duration(constant.graphDurationDimSwitch).attr("transform", function(d, i) {
      return "translate(" + (graphXScale(nationalData.dates[i])) + ", " + (graphYScale(d)) + ")";
    });
  }
  if (firstTime) {
